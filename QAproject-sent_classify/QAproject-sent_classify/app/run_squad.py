...
"""Script for KorBERT WEB SERVING"""
# JAEHAK
# 20210420 
...
def read_squad_examples(input_file, is_training):
  ...
  tf_record로 training data를 변환하기 전,
  input_file을 읽어 examples 라는 리스트에 저장하는 과정입니다.
  """
  ...
  for entry in input_data:
    for paragraph in entry["paragraphs"]:
      ...
      # 띄어쓰기를 기반으로 토큰화된 doc_tokens 리스트를 만듭니다.
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  만들어진 examples 파일을 배치 사이즈로 나누고 
  feature 파일로 만드는 과정입니다. 
  """
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  start_logits, end_logits를 산출하는 모델을 만드는 과정입니다.
  """
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    ##################### 조사 분리 #######################
      ...
      #changed_num = len(nbest_json[0]["text"]) - len(new_answer)
      all_predictions[example.qas_id] = (new_answer, nbest_json[0]["start"], nbest_json[0]["end"])
    #######################################################
